...
class DiceMetric(CumulativeIterationMetric):
    ...
    def _compute_tensor(self, y_pred: paddle.Tensor, y: paddle.Tensor):  # type: ignore
        """
        Args:
...
def compute_meandice(y_pred: paddle.Tensor, y: paddle.Tensor, include_background: bool = True) -> paddle.Tensor:
    ...
    intersection = paddle.sum(y * y_pred, axis=reduce_axis)

    y_o = paddle.sum(y, reduce_axis)
    y_pred_o = paddle.sum(y_pred, axis=reduce_axis)
    denominator = y_o + y_pred_o

    return paddle.where(y_o > 0, (2.0 * intersection) / denominator, paddle.to_tensor(float("0")))
