...
class IterationMetric(Metric):
    ...
    def __call__(self, y_pred: TensorOrList, y: Optional[TensorOrList] = None):  # type: ignore
        ...
        if isinstance(y_pred, paddle.Tensor):
            y_ = y if isinstance(y, paddle.Tensor) else None
            return self._compute_tensor(y_pred, y_)
    ...
    def _compute_list(self, y_pred: TensorOrList, y: Optional[TensorOrList] = None):
        ...
        """
        if y is not None:
            ret = [self._compute_tensor(p.unsqueeze(0), y_.unsqueeze(0)) for p, y_ in zip(y_pred, y)]
        else:
            ret = [self._compute_tensor(p_.unsqueeze(0), None) for p_ in y_pred]

        # concat the list of results (e.g. a batch of evaluation scores)
        if isinstance(ret[0], paddle.Tensor):
            return paddle.concat(ret, axis=0)
        # the result is a list of sequence of tensors (e.g. a batch of multi-class results)
        if isinstance(ret[0], (list, tuple)) and all(isinstance(i, paddle.Tensor) for i in ret[0]):
            return [paddle.concat(batch_i, axis=0) for batch_i in zip(*ret)]
        return ret

    @abstractmethod
    def _compute_tensor(self, y_pred: paddle.Tensor, y: Optional[paddle.Tensor] = None):
        """
...
class Cumulative:
    ...
    def __init__(self):
        ...
        """
        self._buffers: Optional[List[List[paddle.Tensor]]] = None
        self._synced_tensors: Optional[List[Optional[paddle.Tensor]]] = None
    ...
    def extend(self, *data) -> None:
        ...
        for b, d in zip(self._buffers, data):
            ...
            d_t: paddle.Tensor
            d_t, *_ = convert_data_type(d, output_type=paddle.Tensor, wrap_sequence=True)  # type: ignore
            try:
                b.extend([x[0] for x in paddle.split(d_t, 1, axis=0)])
