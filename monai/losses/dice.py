    ...
    def forward(self, input: paddle.Tensor, target: paddle.Tensor) -> paddle.Tensor:
        ...
        if self.to_onehot_y:
            if n_pred_ch == 1:
                ...
                target = utils.one_hot(target, num_classes=n_pred_ch)
        ...
        reduce_axis: List[int] = paddle.arange(2, len(input.shape)).tolist()
        if self.batch:
            # reducing spatial dimensions and batch
            reduce_axis = [0] + reduce_axis

        intersection = paddle.sum(target * input, axis=reduce_axis)

        if self.squared_pred:
            target = paddle.pow(target, 2)
            input = paddle.pow(input, 2)

        ground_o = paddle.sum(target, axis=reduce_axis)
        pred_o = paddle.sum(input, axis=reduce_axis)
        ...
        f: paddle.Tensor = 1.0 - (2.0 * intersection + self.smooth_nr) / (denominator + self.smooth_dr)

        if self.reduction == "mean":
            f = paddle.mean(f)  # the batch and channel average
        elif self.reduction == "sum":
            f = paddle.sum(f)  # sum over the batch and channel dims
...
class DiceCELoss(nn.Layer):
    ...
    def __init__(
        ...
        smooth_dr: float = 1e-5,
        batch: bool = False,
        ce_weight: Optional[paddle.Tensor] = None,
    ...
    def forward(self, input: paddle.Tensor, target: paddle.Tensor) -> paddle.Tensor:
        ...
        total_loss: paddle.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss
